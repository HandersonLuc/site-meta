...
export function OtherServices({ currentCoordSlug }: { currentCoordSlug: string }) {
  const otherCoords = coordinations.filter((coord) => coord.slug !== currentCoordSlug);

  return (
    <section className="bg-[#090E28] text-white py-24">
      <div className="container mx-auto px-4 text-center">
        <h2 className="text-2xl md:text-3xl font-bold mb-12 border-b-2 border-[#2AD8FF] inline-block">
        ...
        <div className="flex flex-col items-center gap-8 max-w-5xl mx-auto">
          ...
          <div className="flex flex-wrap justify-center gap-8">
            {otherCoords.slice(0, 2).map((coord) => (
              <Link
                key={coord.slug}
          ...
          <div className="flex flex-wrap justify-center gap-8">
            {otherCoords.slice(2, 5).map((coord) => (
              <Link
                key={coord.slug}
                href={`/servicos/${coord.slug}`}
                className="group w-56 rounded-xl overflow-hidden shadow-lg hover:shadow-xl transition-all duration-300 border border-[#2AD8FF]/20"
